...
int binary(int start, int end, int next_block, int num, int result_location, int result_length);
...
//二元搜索
int binary(int start, int end, int next_block,int num,int result_location, int result_length = 0) {
	unsigned char *blk;
	Buffer buf;
	unsigned char *result_blk;
	...
	if (next_block >= start && next_block <= end) {
		if ((blk = readBlockFromDisk(next_block, &buf)) == NULL) {
			perror("Reading Block Failed!\n");
		}
		...
		if (min_inblock > num) {
			...
			next_block = (end + start) / 2;
			freeBlockInBuffer(blk, &buf);
			tmp_num1 = binary(start, end, next_block, num,result_location,result_length);
		}
		if (max_inblock < num) {
			start = next_block + 1;
			next_block = (start + end) / 2;
			freeBlockInBuffer(blk, &buf);
			tmp_num2 = binary(start,end,next_block,num,result_location,result_length);
		}
		// 如果要查询的数只可能在这个块内
		if (min_inblock < num && max_inblock > num) {
			for (i = 1; i < 6; i++) {
				for (j = 0; j < 4; j++) {
					str[j] = *(blk + 8 * i + j);
				}
		...
		else if (min_inblock == num && max_inblock != num) {
			for (i = 0; i < 7; i++) {
				for (j = 0; j < 4; j++) {
					str[j] = *(blk + 8 * i + j);
				}
			...
			next_block--;
			tmp_num3 = binary(start,end,next_block,num,result_location,result_length);
			freeBlockInBuffer(blk, &buf);
		}
		else if (min_inblock != num && max_inblock == num) {
			for (i = 0; i < 7; i++) {
			...
			start = next_block + 1;
			next_block++;
			tmp_num4 = binary(start, end, next_block, num, result_location, result_length);
			freeBlockInBuffer(blk, &buf);
		}
		else if (min_inblock == num && max_inblock == num) {
		for (i = 0; i < 7; i++) {
		...
		tmp_num5 = binary(start, end - 1, next_block - 1, num, result_location, result_length);
		tmp_num6 = binary(start+1, end, next_block+1, num, result_location, result_length);
		freeBlockInBuffer(blk, &buf);
		}
	}
	tmp_num = buf.numIO + tmp_num1 + tmp_num2 + tmp_num3 + tmp_num4 + tmp_num5 + tmp_num6;
	return tmp_num;
}
...
void binary_select() {
	...
	memset(result_blk, 0, 64);
	printf("\n\n二元搜索：\n");
	tmp_num = binary(start, end, next_block, 40, result_location,0);
/*
	while (next_block >= start && next_block <= end) {
		if ((blk = readBlockFromDisk(next_block, &buf)) == NULL) {
			perror("Reading Block Failed!\n");
		}
...
*/
	...
	start = 220, end=252;
	next_block = 235;
	tmp_num1 = binary(start, end, next_block, 60, result_location,0);
/*
